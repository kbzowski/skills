...
name: refactor
description: >
  Codebase refactoring with focus on readability, simplification, strong typing,
  and framework best practices. Use when the user asks to refactor, decompose, simplify,
  deduplicate, or improve code quality. Triggers: "refaktoryzacja", "refactor", "uprość",
  "dekompozycja", "deduplikacja", "wydziel komponent", "popraw typowanie", "iteracja
  refaktoryzacji", "posprzątaj", "ten plik jest za duży", "tu jest bałagan", "popraw
  architekturę", "rozbij na mniejsze", "wydziel serwis", "uprość API". Covers: component
  extraction, hook extraction, type consolidation, service layer deduplication, design
  patterns, module reorganization, internal API redesign, directory restructuring,
  server/DB query optimization, and framework-specific best practices.
---
...
Refactoring is not cosmetic cleanup. The goal is to make the codebase structurally better —
simpler to reason about, easier to extend, harder to misuse. This means touching core modules,
changing internal APIs, moving files between directories, and removing unnecessary abstractions.

Do not be timid. Small renames and extract-variable changes are fine as part of a larger effort,
but they should never be the entire deliverable. If a module has deep structural problems, address
them — do not paper over them with surface-level tidying.

When in doubt, prefer a bold change that simplifies the architecture over a safe change that
preserves the status quo. The build/lint/test verification step exists precisely to catch
regressions from ambitious changes.
...
3. **Look up current best practices** — for each major framework/library in the stack, search
   the web for its current documentation. Focus on migration guides, recommended patterns, and
   deprecation notices. Do not rely on memorized patterns — they may be outdated.
...
Wait for user approval or scope adjustment before proceeding.
...
(as documented in CLAUDE.md). Typical sequence:
- Build
- Lint / format check
...
Save to `docs/refactor-reports/iteration-N.md` (create directory if needed).
...
3. **Strong static typing** — prefer type deduction, `typeof`, `ReturnType`, schema inference
   over explicit generics, class inheritance, or escape hatches like `any`. Reach for generics
   and inheritance only when deduction yields something worse
...
use the ones relevant to the target area. Each lens describes what to look for and what
good looks like after refactoring.
...
- Inline logic in JSX that obscures the render tree
...
- Type deduction preferred (`typeof`, `ReturnType`, `Awaited`, schema inference)
...
- Barrel files (index.ts re-exports) that obscure the real dependency graph
...
- **Redesign internal APIs** — change function signatures, hook return types, component prop
...
- **Consolidate duplicated types** — find structurally identical or near-identical types across
  the codebase and unify them. Derive variants with Pick, Omit, or schema transforms.
...
After completing all changes, generate a report following
